let CACHE_VERSION = STUB_CACHE_VERSION;
...
function getGitHubUrl(path, ref) {
	return GITHUB_API_URL + (ref || '').replace(/\W+/g, '');
}

async function getGitHubResponse(path, ref) {
	var githubUrl = getGitHubUrl(path, ref);
...
function cacheDelete(request) {
	return caches.open(CACHE_VERSION).then(cache => cache.delete(request));
}
